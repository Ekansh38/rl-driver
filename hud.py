...
class HUD:
    ...
    def _draw_graphs(self, screen, lap_timer, telemetry):
        ...
        nav_y = py + panel_h - 26
        if self.graph_idx > 0:
            lbl = self.font_label.render(f"<- {GRAPH_NAMES[self.graph_idx - 1]}", True, (120, 120, 120))
    ...
    def _draw_lap_times_graph(self, screen, gx, gy, gw, gh, lap_timer):
        ...
        if len(laps) < 2:
            ...
            screen.blit(msg, (gx + gw // 2 - msg.get_width() // 2, gy + gh // 2 - msg.get_height() // 2))
            return
        ...
        for i, (t, pt) in enumerate(zip(laps, points)):
            ...
            color = (255, 215, 0) if is_best else (220, 80, 80) if is_last else (90, 90, 90)
            r = 5 if (is_best or is_last) else 3
            pygame.draw.circle(screen, color, pt, r)

        # labels for best and last
        for i, color, offset_y in [(best_idx, (255, 215, 0), -20), (len(laps) - 1, (220, 80, 80), 8)]:
    ...
    def _draw_speed_profile_graph(self, screen, gx, gy, gw, gh, lap_timer, telemetry):
        ...
        if best_tele:
            ...
            pts_b = [to_pt(i, len(best_tele), best_tele[i][0]) for i in range(0, len(best_tele), step_b)]
            if len(pts_b) >= 2:
        ...
        for i in range(0, total - step, step):
            ...
            pt1 = to_pt(min(i + step, total - 1), total, last_tele[min(i + step, total - 1)][0])
            color = (80, 220, 80) if accel else (220, 80, 80) if brake else (160, 160, 160)
            pygame.draw.line(screen, color, pt0, pt1, 2)

        # legend
        lx = gx + gw - 130
        ly = gy + 10
        for text, color in [("ACCEL", (80, 220, 80)), ("BRAKE", (220, 80, 80)), ("COAST", (160, 160, 160))]:
